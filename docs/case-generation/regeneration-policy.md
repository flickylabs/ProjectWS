...
### §3.1 archived `docs/LEGACY/20260506-pre-steam-cleanup/scripts/assemble-family-friend-bundles.cjs` - STALE / NOT CURRENT CANONICAL

Archived `docs/LEGACY/20260506-pre-steam-cleanup/scripts/assemble-family-friend-bundles.cjs`는 현재 complete canonical regeneration pathway로 취급하면 안 된다.

- Scope: `family-01`, `friend-01`만 포함한다. `spouse-01` config는 없다.
- Expected inputs: 각 사건별 9개 session JSON.
  - archived `docs/LEGACY/20260506-pre-steam-cleanup/ref/리뉴얼참고/gpt-pro-prompts-family/output/`
  - archived `docs/LEGACY/20260506-pre-steam-cleanup/ref/리뉴얼참고/gpt-pro-prompts-friend/output/`
...
- Current inputs: 위 archived `docs/LEGACY/20260506-pre-steam-cleanup/ref/리뉴얼참고/gpt-pro-prompts-{family,friend}/output/` directories는 현재 active working tree에 없다.
...
WARNING: Do not run the archived `assemble-family-friend-bundles.cjs` as-is. If the historical input directories are restored and this script is run, it can overwrite the current 18-channel runtime bundles with 6-channel bundles and drop registered manual fixes.
...
`spouse-01`은 archived `assemble-family-friend-bundles.cjs`의 CASE_CONFIGS에 포함되어 있지 않다. Phase 1 조사 기준으로 `spouse-01`의 canonical regeneration pathway는 이 문서에서 확정하지 않는다.
...
### §4.1 archived `docs/LEGACY/20260506-pre-steam-cleanup/ref/리뉴얼참고/gpt-pro-prompts-{family,friend}/output/`

다음 directories는 historical input surface다. 현재 working tree에는 존재하지 않는다.

| Path | Current status | Git history summary |
|---|---|---|
| `docs/LEGACY/20260506-pre-steam-cleanup/ref/리뉴얼참고/gpt-pro-prompts-family/output/` | absent from active tree | `8625986`에서 prompt/output files 추가, `574d563`에서 삭제 |
| `docs/LEGACY/20260506-pre-steam-cleanup/ref/리뉴얼참고/gpt-pro-prompts-friend/output/` | absent from active tree | `8625986`에서 prompt/output files 추가, `574d563`에서 삭제 |
...
| Source | GPT Pro-authored adjacent variant text reused from archived `LEGACY/20260506-pre-steam-cleanup/root/gpt-pro-runs/judge-messages-v3/_master/assets-friend-01/gpt-pro-package/` project artifacts |
| Baseline tag | `baseline-pre-policy-v3-track-a-friend-01` -> `8e07b0a` |
| Result docs | archived `LEGACY/20260506-pre-steam-cleanup/tmp/qa-thread-guides/24-CODEX-DEV-TRACK-A-FRIEND-01-RESULT.md`, `LEGACY/20260506-pre-steam-cleanup/tmp/qa-thread-guides/26-POST-PUSH-CONFIRMATION-TRACK-A-FRIEND-01-RESULT.md` |
...
| Date | 2026-04-29 |
| Investigation result | archived `LEGACY/20260506-pre-steam-cleanup/tmp/qa-thread-guides/42-FAMILY-01-SCOPE-DECISION-REDO-RESULT.md` |
| Investigation prior | archived `LEGACY/20260506-pre-steam-cleanup/tmp/qa-thread-guides/34-TRACK-A-A1-C3-DOSSIER-ID-ALIGNMENT-RESULT.md` (§2 family-01) |
...
`spouse-01` is not generated by the archived `assemble-family-friend-bundles.cjs`. Any future `spouse-01` manual fix must still be registered here, but its generator/pathway note must identify the separate canonical pathway used for that case.

## §6 Regeneration Policy

Any future regeneration of `src/data/scriptedText/*.json` must follow this 5-step process.

1. Identify current canonical pathway. Do not use the archived `assemble-family-friend-bundles.cjs` as-is. If the pathway is unknown, stop and document the gap before changing runtime data.
...
`family-01` and `friend-01` share the same stale assembler scope in archived `assemble-family-friend-bundles.cjs` and the same missing historical input-dir pattern under archived `docs/LEGACY/20260506-pre-steam-cleanup/ref/리뉴얼참고/gpt-pro-prompts-{family,friend}/output/`.
...
- Deprecate, update, or replace the archived `assemble-family-friend-bundles.cjs` with a new current script.
- Decide whether to restore archived `docs/LEGACY/20260506-pre-steam-cleanup/ref/리뉴얼참고/gpt-pro-prompts-{family,friend}/output/`.
