...
```json
{
  "spouse-01": {
    "d-1": {
      ...
      "triggerMapping": { "fact_pursuit": "direct_question_about_transfer", "evidence_present:e-1": "e-1_presented" },
      "pathBonus": 8
    },
    "d-2": {
      "pathGoal": "collapse_lie",
      ...
      "triggerMapping": { "fact_pursuit": "phone_access_question" },
      "pathBonus": 6
    },
    "d-3": {
      "pathGoal": "prove_false",
      "requiredActions": ["evidence_present:e-2", "investigate_only:e-4"],
      "bonusActions": ["empathy_approach"],
      "triggerMapping": { "evidence_present:e-2": "e-2_presented" },
      "pathBonus": 8
    }
  }
}
...
- **pathGoal** (필수, 4종 중 1개):
  - `collapse_lie`: 거짓말을 붕괴시키는 것이 목표 (대부분의 쟁점)
  - `prove_false`: 상대 주장이 거짓임을 밝히는 것이 목표 (truth=false인 쟁점)
  - `reveal_mutual_breach`: 양쪽 모두의 위반을 드러내는 것이 목표
  - `keep_pending`: 모호한 쟁점을 보류하는 것이 적절한 판단
- requiredActions: 반드시 거쳐야 하는 액션
- bonusActions: 추가 거치면 보너스 (스킬 포함 가능)
- **triggerMapping**: 각 액션이 lieConfig의 어떤 transition trigger에 매핑되는지 (없으면 생략)
- pathBonus: 경로 완수 시 추가 점수 (6~10)
- 액션 타입:
  - `evidence_present:e-1`: 증거 제시
  - `investigate_only:e-4`: 조사만 (제시 없이). 합법성 문제가 있는 증거는 이 타입 사용
  - `present_if_lawful:e-3`: 합법적일 때만 제시. privacy_concern 증거에 사용
  - `use_confidential_channel`: 비공개 보호 경로로 정보 획득
...
- **절대 규칙**: 사건의 최종 진실을 한 문장으로 요약하지 마라. 증인은 자기가 직접 아는 범위만 말한다.
- bias가 있어도 노골적 결론("그쪽이 맞아요")이 아니라 표현의 선택/강조/생략으로만 드러낸다.
- distortionRisk가 intentional/strategic인 증인은 불리한 부분을 빠뜨리거나 순서를 바꿔야 한다.
- 플레이어가 증인 말만 듣고 조사 없이 정답을 얻을 수 없어야 한다.
...
9. optimalPath의 **pathGoal**은 반드시 `collapse_lie | prove_false | reveal_mutual_breach | keep_pending` 중 1개.
10. optimalPath에 **triggerMapping**을 포함. 각 액션이 lieConfig의 어떤 transition trigger에 매핑되는지 명시.
11. 합법성 문제가 있는 증거는 `investigate_only` 또는 `present_if_lawful` 타입 사용.
12. witnessSpeechSample은 **사건의 최종 진실을 한 문장으로 요약하지 말라**. 증인이 직접 아는 범위만.
