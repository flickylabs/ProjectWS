# Thread: SCRIPT_ZH_CN Intake

Target: ClaudeCode

You are preparing Simplified Chinese script/dialogue localization for `Project_Solomon`. This is an intake and pilot pass only. Do not perform full bulk translation yet.
...
UI localization and glossary consistency are still in progress. Starting script work now is useful, but full translation before the UI glossary stabilizes would create avoidable rework. Your job is to map the script surface, find glossary risks, and produce a small representative Simplified Chinese pilot.
...
## Simplified Chinese Script Style

- Natural Simplified Chinese game dialogue.
- Avoid Traditional Chinese characters and Taiwan/Hong Kong-specific terms.
- Preserve emotional subtext, concealment, hesitation, and reveal pacing.
- Avoid stiff legal wording unless the speaker intentionally uses formal/legal language.
...
- Prefer `认命` for resigned/fatalistic emotion unless local context clearly requires a different term.
...
Describe how Simplified Chinese script localization should eventually be stored and applied without breaking IDs, references, or runtime loaders.
...
```csv
source_ko,proposed_zh_cn,context,path_or_key,relation_to_ui_glossary,risk,notes
```
...
Provide structured samples with original Korean and proposed Simplified Chinese. Preserve source path, case ID, key, speaker, channel, and variant IDs where available.
...
```csv
path,key_or_id,original_ko,proposed_zh_cn,risk,recommendation
```

Flag any sample where Simplified Chinese phrasing could accidentally reveal hidden truth too early.
