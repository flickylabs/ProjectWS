# Thread: SCRIPT_EN Intake

Target: ClaudeCode

You are preparing English script/dialogue localization for `Project_Solomon`. This is an intake and pilot pass only. Do not perform full bulk translation yet.
...
UI localization and glossary consistency are still in progress. Starting script work now is useful, but full translation before the UI glossary stabilizes would create avoidable rework. Your job is to map the script surface, find glossary risks, and produce a small representative English pilot.
...
- Natural courtroom investigation game prose.
- Keep dialogue playable and emotionally readable, not literal.
- Avoid stiff legalese unless a speaker is intentionally formal.
- Preserve character intent, hesitation, concealment, and reveal pacing.
- Preserve truth boundaries. Do not make a line reveal more than the Korean line reveals.
- Preserve speaker relationship and tone.
- Keep repeated variants varied but equivalent.
- Keep system text concise and UI-safe.
...
Describe how English script localization should eventually be stored and applied without breaking IDs, references, or runtime loaders.
...
```csv
source_ko,proposed_en,context,path_or_key,relation_to_ui_glossary,risk,notes
```
...
Provide structured samples with original Korean and proposed English. Preserve source path, case ID, key, speaker, channel, and variant IDs where available.
...
```csv
path,key_or_id,original_ko,proposed_en,risk,recommendation
```

Flag any sample where English phrasing could accidentally reveal hidden truth too early.
