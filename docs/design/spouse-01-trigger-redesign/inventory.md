# spouse-01 narrative trigger 전수 매트릭스 (재설계 진입 전 스냅샷)
...
총 16 entity 가 `narrativeTriggers` 부착. 외도 의심(d-1) / 비자금 사용처(d-2) / 「영수증 묶음 5장」(e-1) / 「블랙박스 GPS 기록」(e-2) / 「통화기록」(e-3) 은 narrative trigger 미부착 (의도 확인 필요).
...
| e-4 | 발신자 미상 문자 | d-1 | e-3 | S1 | 4 | (C-2 hook 결과) |
...
| e-8 | 「헤라 여성 메디컬 센터」 주차 영수증 (발급처 오염) | d-2 + d-3 | — | — | 4 | — |
| e-9 | 산전우울증 자가진단 + 상담 예약 | d-2 + d-3 | — | — | 4 | — |
| e-10 | 「부모가 되는 마음 연습」 출산 준비 도서 | d-2 + d-3 | — | — | 1 (inline in case.ts) | C-3c (e-1 stage 3 → e-10) |
...
각 chain 은 `cascade_from_card` priorCard 연결 그래프. `→` = priorCard 의존성. `⇒` = dual emergence (「출산 준비 도서」 → 「내연녀 임신 의심」 동시).
...
dc-1 (오피스텔의 사람들)
  ...
  ├─→ w-1 (오피스텔 경비)
  └─→ dc-2 (시댁 얘기만 나오면 싸움, cross-line: d-1 + d-2)

별도 chain: e-1 (영수증 묶음 5장) / e-2 (블랙박스 GPS 기록) — narrative trigger 0, cascade chain 미참여
```
...
h-d3 (위임장 조작 hidden dispute) — narrative trigger 4 candidate (combine-5 / d-2 S2+ + b interject / a outburst / judge auto)
```
...
전제: 비자금 사용처(d-2) S5+ 도달 (비자금 사용처 자백 = 형 사정). 「내연녀 임신 의심」(d-3) 영향 X (parallel chain).
```
...
| e5-via-judge-auto-mention | judge_auto_mention | `turnsAfterEligible: 5` (dispute 없음) | 2 refs | standard |
...
| dc3-via-judge-auto | judge_auto_mention | `turnsAfterEligible: 5` (dispute 없음) | 2 refs | standard |
...
### 비자금 진실 line (h-d4 = 난임 치료비 진실) — 4 entity (e-8 / e-9 / dc-8 / h-d4)

#### e-8 「헤라 여성 메디컬 센터」 주차 영수증 — 4 candidate
...
#### e-10 「부모가 되는 마음 연습」 출산 준비 도서 — 1 candidate (inline in case.ts line 2536~2552)
...
## Part 4: useActionDispatch.ts Hybrid hook

schema stage gate 한계 보완을 위한 runtime hook 2개.

### Hook C-2 (line 1206~1252) — 「통화기록」(e-3) stage 2 → 「발신자 미상 문자」(e-4) cascade
...
결과:
  ...
  4. setPostCascadeV4FallbackMute('b', 'd-1', state.turnCount)
       ...
       → hard_evidence / contradiction_pursuit / witness 는 자연 진행
```

### Hook C-3c (line 1158~1204) — 「영수증 묶음 5장」(e-1) stage 3 → 「출산 준비 도서」(e-10) cascade
...
결과:
  ...
  4. e-10 fire 시 d-3 (내연녀 임신 의심) narrativeTrigger 'd3-via-cascade-from-e10' 자동 fire (dual emergence)
     → d-3 동시 surface — misdirection frame 표면화
```
...
| type | 개수 | 비고 |
|---|---|---|
| cascade_from_card | ~22 | priorCard chain 의존. 가장 큰 type. |
| npc_interjection | ~14 | 배열 OR 형식의 정밀도가 다양 (broad / 정밀 mix). |
| combination_result | 7 | recipeId 의존 (combine-1/3/4/5/6/7 + clue-cash-pattern). |
| emotional_outburst | 8 | partyPhase 정밀 (대부분 shaken / angry / resigned 조합). |
...
**비자금 진실 line (h-d4 = 난임 치료비 진실)** = 본 사건 신설 line. 「주차 영수증」(e-8) / 「산전우울증 자가진단」(e-9) / 「이준호의 또 다른 침묵」(dc-8) / h-d4 = "사실은 B 비자금이 박지연 난임 치료비" 진실. 이준호 가족 사정 line 과는 별개로 추가된 진실 line.

**misdirection (d-3 = 내연녀 임신 의심)** = "박지연이 내연녀 출산 대비로 misdirect" — 「출산 준비 도서」(e-10) 등장 시 misdirection frame 표면화. d-3 무너짐 → h-d4 (난임 치료비 진실) 수렴.

재설계 시 검토 포인트:
- **Chain 1 ↔ Chain 5 연결** = 외도 의심 line 조사 중 「영수증 묶음」(e-1) stage 3 + b 제시 → 「출산 준비 도서」(e-10) cascade → 「내연녀 임신 의심」(d-3) dual surface. **이 연결이 사용자 보기 부자연** (영수증에서 책이 나오는 흐름).
- **Chain 4 (비자금 진실) 진입 조건** = 비자금 사용처(d-2) S5+ (비자금 사용처 자백). 즉 위임장 line 조사 + d-2 S5 도달 후만 진입. 「내연녀 임신 의심」(d-3) 영향 X. → d-3 무너짐 chain 과 비자금 진실 chain 이 독립.
- **이중 진입** = 「출산 준비 도서」(e-10) cascade 로 「내연녀 임신 의심」(d-3) surface 하면 d-3 S5 successUnlocks: ['h-d4'] + d-2 S5+ 비자금 chain — 2 path 로 h-d4 surface 가능. 사용자 보기 어느 쪽이 우선?
