# Codex Thread — family-01 interrogation Batch C 잔존 패턴 63 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/family01-interrogation-C-sync ../ws-family01-interrogation-C-sync main` |
...
| 산출 | branch `codex/family01-interrogation-C-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/family-01.json`의 `interrogation` channel **63 entries** (commit on main: `faa80887`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 완전히 + 그_부분 + 그_판단 + 차분히 + 진짜 + 느껴집니다 잔존 패턴 묶음. 63 unique entries 1:1 polish.
- NPC 자기 진술 channel. 다중 패턴 자연화.
...
- KO file — main에 이미 적용 완료 (commit `faa80887`)
- 다른 batch — 별도 의뢰서
...
main commit `faa80887`의 `git show faa80887` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show faa80887 -- src/data/scriptedText/family-01.json
```
...
- worktree branch `codex/family01-interrogation-C-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/family01-interrogation-C-sync
  Commit: <hash>
