# Codex Thread — friend-01 small channel 어색 잔존 58 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/friend01-small-residual-sync ../ws-friend01-small-residual-sync main` |
...
| 산출 | branch `codex/friend01-small-residual-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/friend-01.json`의 **friend-01 small channel 어색 잔존 58 entries** (commit on main: `4075d40c`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 10개 채널 통합 (judge_* 제외). NPC 자기 발화 영역.
- 채널: interrogation 14 / mediation 8 / aftermath 7 / trust_action 7 / emotional_overload 6 / rapport_milestone 6 / contradiction_pursuit 4 / interjection 3 / witness 2 / evidence_discovery 1. NPC 자기지시 "본인/자신" → "저/제" 자연화 영역. 호명 보존.
- 채널: 10개 NPC 발화 채널
...
- KO file — main에 이미 적용 완료 (commit `4075d40c`)
- 다른 batch — 별도 의뢰서
...
main commit `4075d40c`의 `git show 4075d40c` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 4075d40c -- src/data/scriptedText/friend-01.json
```
...
- worktree branch `codex/friend01-small-residual-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/friend01-small-residual-sync
  Commit: <hash>
