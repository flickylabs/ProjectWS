# Codex Thread β5 — LQA Phase 2 / family-01 / Japanese (ja)
...
주체: Codex thread β5 (병렬 10 thread 중 하나)
...
| 항목 | 조건 |
|---|---|
| **shared worktree 안전 규칙** | 본 thread는 read-only on `src/`. **어떤 `src/` 파일도 수정 금지.** 출력은 오직 `docs/design/translation-lqa-phase/reports/family-01_ja.csv` 1개 파일. |
...
- **케이스**: `family-01`
- **언어**: `Japanese` (`ja`)
- **출력 단일 파일**: `docs/design/translation-lqa-phase/reports/family-01_ja.csv`

### 1.2. 검수 대상 source 파일 (READ-ONLY)

KO baseline (비교 기준):
- `src/data/cases/generated/family-01.json`
- `src/data/scriptedAngles/family-01_angle_catalog.json`
- `src/data/scriptedAngles/family-01_judge_questions.json`
- `src/data/scriptedText/family-01.json`

Japanese 검수 대상:
- `src/data/cases/generated/family-01.ja.json`
- `src/data/dialogues/mediation/family-v3-01.ja.json`
- `src/data/dialogues/phase1/family-01.ja.json`
...
표본 추출 시 seed = `family-01-ja-2026-05-19` 사용 (재현성). `node -e "..."` 로 randomization 시 `Math.seedrandom` 또는 hash 기반 stratification.
...
`docs/design/translation-lqa-phase/reports/family-01_ja.csv` 파일 형식:
...
| docs/design/translation-lqa-phase/reports/family-01_ja.csv 외 다른 파일 staged/modified 0건 | ✓ |

---

## §5. 종료 시 작업

1. CSV 마무리 (`docs/design/translation-lqa-phase/reports/family-01_ja.csv` 저장)
2. `git status --short` 실행 → 출력 확인
3. `git add docs/design/translation-lqa-phase/reports/family-01_ja.csv` (다른 파일 add 금지)
4. commit 메시지:
   ```
   LQA Phase 2 / family-01 / ja — <thread 종료 시 채울 것: P0/P1/P2 카운트 + hot category>
   
   Thread β5, <thread 종료 시 채울 것> issues found (<채울 것> P0, <채울 것> P1, <채울 것> P2).
   No src/ changes.
