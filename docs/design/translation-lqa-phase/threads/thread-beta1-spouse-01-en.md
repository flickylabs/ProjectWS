# Codex Thread β1 — LQA Phase 2 / spouse-01 / English (en)
...
주체: Codex thread β1 (병렬 10 thread 중 하나)
...
| 항목 | 조건 |
|---|---|
| **shared worktree 안전 규칙** | 본 thread는 read-only on `src/`. **어떤 `src/` 파일도 수정 금지.** 출력은 오직 `docs/design/translation-lqa-phase/reports/spouse-01_en.csv` 1개 파일. |
...
- **케이스**: `spouse-01`
- **언어**: `English` (`en`)
- **출력 단일 파일**: `docs/design/translation-lqa-phase/reports/spouse-01_en.csv`

### 1.2. 검수 대상 source 파일 (READ-ONLY)

KO baseline (비교 기준):
- `src/data/cases/generated/spouse-01.json`
- `src/data/scriptedAngles/spouse-01_angle_catalog.json`
- `src/data/scriptedAngles/spouse-01_judge_questions.json`
...
표본 추출 시 seed = `spouse-01-en-2026-05-19` 사용 (재현성). `node -e "..."` 로 randomization 시 `Math.seedrandom` 또는 hash 기반 stratification.
...
|---|---|---|---|
| D1 | **자연성** | 모국어(English) 화자 입장에서 어색하지 않음. 직역/번역체 회피. | 어색 = P2, 명백한 번역체 = P1 |
...
| D4 | **문화 컨텍스트** | 한국식 표현이 English 자연 표현으로 치환됨. | 부자연 = P2 |
...
### 2.1. en-specific 추가 검출 패턴

- 직역 영문 명사구 사전 위배 (the X figure / X bracket / X track 등 ~30종)
...
`docs/design/translation-lqa-phase/reports/spouse-01_en.csv` 파일 형식:
...
- `severity`: P0 / P1 / P2
- `option_a`: 정정 후보 A (English, 권장안)
- `option_b`: 정정 후보 B (English, 대안 — 미확신 시 빈칸)
...
| docs/design/translation-lqa-phase/reports/spouse-01_en.csv 외 다른 파일 staged/modified 0건 | ✓ |

---

## §5. 종료 시 작업

1. CSV 마무리 (`docs/design/translation-lqa-phase/reports/spouse-01_en.csv` 저장)
2. `git status --short` 실행 → 출력 확인
3. `git add docs/design/translation-lqa-phase/reports/spouse-01_en.csv` (다른 파일 add 금지)
4. commit 메시지:
   ```
   LQA Phase 2 / spouse-01 / en — <thread 종료 시 채울 것: P0/P1/P2 카운트 + hot category>
   
   Thread β1, <thread 종료 시 채울 것> issues found (<채울 것> P0, <채울 것> P1, <채울 것> P2).
   No src/ changes.
