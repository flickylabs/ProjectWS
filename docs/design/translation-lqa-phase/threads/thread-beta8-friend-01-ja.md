# Codex Thread β8 — LQA Phase 2 / friend-01 / Japanese (ja)
...
주체: Codex thread β8 (병렬 10 thread 중 하나)
...
| 항목 | 조건 |
|---|---|
| **shared worktree 안전 규칙** | 본 thread는 read-only on `src/`. **어떤 `src/` 파일도 수정 금지.** 출력은 오직 `docs/design/translation-lqa-phase/reports/friend-01_ja.csv` 1개 파일. |
...
- **케이스**: `friend-01`
- **언어**: `Japanese` (`ja`)
- **출력 단일 파일**: `docs/design/translation-lqa-phase/reports/friend-01_ja.csv`
...
- `src/data/dialogues/mediation/friend-v3-01.ja.json`
- `src/data/dialogues/phase1/friend-01.ja.json`
- `src/data/scriptedAngles/friend-01_angle_catalog.ja.json`
- `src/data/scriptedAngles/friend-01_judge_questions.ja.json`
...
표본 추출 시 seed = `friend-01-ja-2026-05-19` 사용 (재현성). `node -e "..."` 로 randomization 시 `Math.seedrandom` 또는 hash 기반 stratification.
...
|---|---|---|---|
| D1 | **자연성** | 모국어(Japanese) 화자 입장에서 어색하지 않음. 직역/번역체 회피. | 어색 = P2, 명백한 번역체 = P1 |
...
| D4 | **문화 컨텍스트** | 한국식 표현이 Japanese 자연 표현으로 치환됨. | 부자연 = P2 |
...
### 2.1. ja-specific 추가 검출 패턴

- 조사 휴리스틱 위배 (が/は/を 단순 패턴, 명사+を+명사 nonsense 등)
...
`docs/design/translation-lqa-phase/reports/friend-01_ja.csv` 파일 형식:
...
- `target_text`: 검수 대상 Japanese 문자열
...
- `severity`: P0 / P1 / P2
- `option_a`: 정정 후보 A (Japanese, 권장안)
- `option_b`: 정정 후보 B (Japanese, 대안 — 미확신 시 빈칸)
...
| docs/design/translation-lqa-phase/reports/friend-01_ja.csv 외 다른 파일 staged/modified 0건 | ✓ |

---

## §5. 종료 시 작업

1. CSV 마무리 (`docs/design/translation-lqa-phase/reports/friend-01_ja.csv` 저장)
2. `git status --short` 실행 → 출력 확인
3. `git add docs/design/translation-lqa-phase/reports/friend-01_ja.csv` (다른 파일 add 금지)
4. commit 메시지:
   ```
   LQA Phase 2 / friend-01 / ja — <thread 종료 시 채울 것: P0/P1/P2 카운트 + hot category>
   
   Thread β8, <thread 종료 시 채울 것> issues found (<채울 것> P0, <채울 것> P1, <채울 것> P2).
   No src/ changes.
