# Codex Thread {{THREAD_ID}} — LQA Phase 2 / {{CASE}} / {{LANG_NAME}} ({{LANG}})
...
주체: Codex thread {{THREAD_ID}} (병렬 10 thread 중 하나)
...
| 항목 | 조건 |
|---|---|
| **shared worktree 안전 규칙** | 본 thread는 read-only on `src/`. **어떤 `src/` 파일도 수정 금지.** 출력은 오직 `{{OUTPUT_PATH}}` 1개 파일. |
...
표본 추출 시 seed = `{{SAMPLE_SEED}}` 사용 (재현성). `node -e "..."` 로 randomization 시 `Math.seedrandom` 또는 hash 기반 stratification.
...
|---|---|---|---|
| D1 | **자연성** | 모국어({{LANG_NAME}}) 화자 입장에서 어색하지 않음. 직역/번역체 회피. | 어색 = P2, 명백한 번역체 = P1 |
...
| D4 | **문화 컨텍스트** | 한국식 표현이 {{LANG_NAME}} 자연 표현으로 치환됨. | 부자연 = P2 |
...
- `target_text`: 검수 대상 {{LANG_NAME}} 문자열
...
- `severity`: P0 / P1 / P2
- `option_a`: 정정 후보 A ({{LANG_NAME}}, 권장안)
- `option_b`: 정정 후보 B ({{LANG_NAME}}, 대안 — 미확신 시 빈칸)
...
| {{OUTPUT_PATH}} 외 다른 파일 staged/modified 0건 | ✓ |

---

## §5. 종료 시 작업

1. CSV 마무리 (`{{OUTPUT_PATH}}` 저장)
2. `git status --short` 실행 → 출력 확인
3. `git add {{OUTPUT_PATH}}` (다른 파일 add 금지)
4. commit 메시지:
   ```
   LQA Phase 2 / {{CASE}} / {{LANG}} — {{COMMIT_SUMMARY}}
   
   Thread {{THREAD_ID}}, {{ROW_COUNT}} issues found ({{P0_COUNT}} P0, {{P1_COUNT}} P1, {{P2_COUNT}} P2).
   No src/ changes.
