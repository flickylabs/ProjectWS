# Codex Thread — friend-01 interrogation 어색 잔존 125 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/friend01-interrogation-sync ../ws-friend01-interrogation-sync main` |
...
| 산출 | branch `codex/friend01-interrogation-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/friend-01.json`의 `interrogation` channel **125 entries** (commit on main: `fb66e2cf`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 강조부사 "그렇게" 96 dominant. 끝까지 9 + 그_부분 5 잔존. 125 unique entries 1:1 polish.
- NPC 자기 진술 channel. friend-01 (친구 사건). lieState 단계 톤 보존.
...
- KO file — main에 이미 적용 완료 (commit `fb66e2cf`)
- 다른 batch — 별도 의뢰서
...
main commit `fb66e2cf`의 `git show fb66e2cf` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show fb66e2cf -- src/data/scriptedText/friend-01.json
```
...
- worktree branch `codex/friend01-interrogation-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/friend01-interrogation-sync
  Commit: <hash>
