# Codex Thread — friend-01 contradiction_pursuit 어색 잔존 50 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/friend01-contradiction-pursuit-sync ../ws-friend01-contradiction-pursuit-sync main` |
...
| 산출 | branch `codex/friend01-contradiction-pursuit-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/friend-01.json`의 `contradiction_pursuit` channel **50 entries** (commit on main: `3e3d945e`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 그렇게 19 + 끝까지 12 + 그_부분 12 + 진짜 4 + 완전히 2 다중 패턴. 50 unique entries 1:1 polish.
- NPC 모순 지적 channel. 호명 보존 (예비신랑 / 친구). hidden keyword 신규 도입 X.
...
- KO file — main에 이미 적용 완료 (commit `3e3d945e`)
- 다른 batch — 별도 의뢰서
...
main commit `3e3d945e`의 `git show 3e3d945e` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 3e3d945e -- src/data/scriptedText/friend-01.json
```
...
- worktree branch `codex/friend01-contradiction-pursuit-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/friend01-contradiction-pursuit-sync
  Commit: <hash>
