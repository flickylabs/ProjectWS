# Codex Thread — friend-01 judge_* 채널 어색 잔존 18 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/friend01-judge-residual-sync ../ws-friend01-judge-residual-sync main` |
...
| 산출 | branch `codex/friend01-judge-residual-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/friend-01.json`의 **friend-01 judge_* 채널 어색 잔존 18 entries** (commit on main: `42b6ba1c`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 끝까지 + 그렇게 + 차분히 + 자기지시:본인 + 자기지시:자신 + 추상명사 다중 패턴. 18 unique entries 1:1 polish.
- 재판관 → NPC 발화 영역. judge_question 10 + judge_contradiction 5 + judge_witness_summon 3 통합. 자기지시 정책 분리. 호명 보존 (송다은 씨 / 최수민 씨).
- 채널: judge_question / judge_contradiction / judge_witness_summon
...
- KO file — main에 이미 적용 완료 (commit `42b6ba1c`)
- 다른 batch — 별도 의뢰서
...
main commit `42b6ba1c`의 `git show 42b6ba1c` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 42b6ba1c -- src/data/scriptedText/friend-01.json
```
...
judge_question / judge_contradiction / judge_witness_summon
...
- worktree branch `codex/friend01-judge-residual-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/friend01-judge-residual-sync
  Commit: <hash>
