# Codex Thread — spouse-01 judge_* 채널 어색 잔존 20 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/spouse01-judge-residual-sync ../ws-spouse01-judge-residual-sync main` |
...
| 산출 | branch `codex/spouse01-judge-residual-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/spouse-01.json`의 **spouse-01 judge_* 채널 어색 잔존 20 entries** (commit on main: `b0af84fe`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 강조부사:그렇게 8 + 끝까지 5 + 추상명사 5 + 직역체 2 다중 패턴. 20 unique entries 1:1 polish.
- 재판관 → NPC 발화 영역. judge_evidence_combo 9 + judge_question 6 + judge_contradiction 5 통합. 자기지시 "본인" only 정책 부합 영역은 batch 제외 (이미 보존됨). "자신" 영역 본인 통일 (있다면). 호명 보존 (박지연 씨 / 이준호 씨).
- 채널: judge_evidence_combo / judge_question / judge_contradiction
...
- KO file — main에 이미 적용 완료 (commit `b0af84fe`)
- 다른 batch — 별도 의뢰서
...
main commit `b0af84fe`의 `git show b0af84fe` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show b0af84fe -- src/data/scriptedText/spouse-01.json
```
...
judge_evidence_combo / judge_question / judge_contradiction
...
- worktree branch `codex/spouse01-judge-residual-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/spouse01-judge-residual-sync
  Commit: <hash>
