# Codex Thread — family-01 interrogation Batch B "끝까지" 57 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/family01-interrogation-B-sync ../ws-family01-interrogation-B-sync main` |
...
| 산출 | branch `codex/family01-interrogation-B-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/family-01.json`의 `interrogation` channel **57 entries** (commit on main: `66cb2640`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 강조부사 "끝까지" 단일 패턴. 57 unique entries 1:1 polish.
- NPC 자기 진술 channel. "끝까지 ~지 못했습니다" 자연 추궁 영역 보존.
...
- KO file — main에 이미 적용 완료 (commit `66cb2640`)
- 다른 batch — 별도 의뢰서
...
main commit `66cb2640`의 `git show 66cb2640` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 66cb2640 -- src/data/scriptedText/family-01.json
```
...
- worktree branch `codex/family01-interrogation-B-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/family01-interrogation-B-sync
  Commit: <hash>
