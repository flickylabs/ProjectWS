# Codex Thread — spouse-01 contradiction_pursuit 어색 잔존 34 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/spouse01-contradiction-pursuit-sync ../ws-spouse01-contradiction-pursuit-sync main` |
...
| 산출 | branch `codex/spouse01-contradiction-pursuit-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/spouse-01.json`의 `contradiction_pursuit` channel **34 entries** (commit on main: `e9230f41`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 그렇게 18 + 그_부분 7 + 끝까지 4 + 완전히 2 다중 패턴 단일 batch. 34 unique entries 1:1 polish.
- NPC가 상대 모순 지적·추궁 channel. 호명 보존 (제 남편 / 제 아내). hidden keyword 신규 도입 X.
...
- KO file — main에 이미 적용 완료 (commit `e9230f41`)
- 다른 batch — 별도 의뢰서
...
main commit `e9230f41`의 `git show e9230f41` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show e9230f41 -- src/data/scriptedText/spouse-01.json
```
...
- worktree branch `codex/spouse01-contradiction-pursuit-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/spouse01-contradiction-pursuit-sync
  Commit: <hash>
