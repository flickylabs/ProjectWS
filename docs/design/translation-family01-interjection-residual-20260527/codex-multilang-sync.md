# Codex Thread — family-01 interjection 어색 잔존 14 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/family01-interjection-residual-sync ../ws-family01-interjection-residual-sync main` |
...
| 산출 | branch `codex/family01-interjection-residual-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/family-01.json`의 `interjection` channel **interjection 어색 잔존 14 entries** (commit on main: `51d475ed`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 그_부분 7 + 그렇게 5 + 끝까지 3 다중 패턴. 14 unique entries 1:1 polish.
- NPC 끼어듦 채널. "그 부분" → 구체 referent (가족이 버텨 온 말/숨겨 온 말/이유). major/minor interjection 톤 보존 (방어/항의/이유 평가). 윤정후/형 직접 호명 보존.
...
- KO file — main에 이미 적용 완료 (commit `51d475ed`)
- 다른 batch — 별도 의뢰서
...
main commit `51d475ed`의 `git show 51d475ed` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 51d475ed -- src/data/scriptedText/family-01.json
```
...
NPC가 재판관/시스템 발화 중 짧게 끼어드는 채널 (자기 발화 영역). 톤 5 차원 (격앙/거부/방어/항의/감정 호소).
...
- worktree branch `codex/family01-interjection-residual-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/family01-interjection-residual-sync
  Commit: <hash>
