# Codex Thread — spouse-01 evidence_present Batch 6 동형 archetype 245 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/spouse01-evidence-batch6-sync ../ws-spouse01-evidence-batch6-sync main` |
...
| 산출 | branch `codex/spouse01-evidence-batch6-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/spouse-01.json`의 `evidence_present` channel **245 entries** (commit on main: `1792ffb0`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- B5와 동일 구조. 30 archetypes → 49 archetypes × 5 evidence = 245 entries 일괄. party=B (이준호 defendant) 48 + 박지연 1 archetype 잔존.
- ★ late-stage2/3 (10 archetype) "가족 / 가족 문제 / 집안 사정" 영역 등장 — 자백 frame 보존 OK. "형/회생/형사/사기/횡령" 구체 hidden keyword 신규 도입 X. "외도" 인용 표현 보존 OK.
...
- KO file — main에 이미 적용 완료 (commit `1792ffb0`)
- 다른 batch — 별도 의뢰서
...
main commit `1792ffb0`의 `git show 1792ffb0` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 1792ffb0 -- src/data/scriptedText/spouse-01.json
```
...
- worktree branch `codex/spouse01-evidence-batch6-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/spouse01-evidence-batch6-sync
  Commit: <hash>
