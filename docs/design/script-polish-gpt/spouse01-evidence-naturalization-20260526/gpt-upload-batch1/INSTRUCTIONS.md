# GPT Pro 작업 지시서 — spouse-01 evidence_present Batch 1 (e-1+e-2+e-3) 자연화

## 당신의 역할

당신은 한국어 게임 시나리오 자연화 전문가입니다. 본 폴더의 file들을 정확히 읽고 polish 작업을 수행해 주세요.

**★ 본 batch는 NPC 발화 영역** (재판관 발화 X). NPC가 evidence 제시받고 반응하는 자기 진술. 어색 패턴이 사전 검출된 50 variants 자연화.
...
- **`batch-01-e1-e2-e3.md`** — Batch 1 (e-1 학원생 참고서 / e-2 GPS / e-3 통화기록, 50 variants 자연화)

batch file은 다음 정보 포함:
- Evidence Frame (e-1/e-2/e-3 각 evidence 의미 + 진실 영역)
- lieBand · stage 의미 가이드 (early 방어 / mid 변명 / late 진실 인정)
- 변경 대상 50 variants 표 (ID / party / lieBand / stage / 어색 패턴 / KO 원본)
...
6. lieBand 단계 톤 보존
7. band=late 단계 hidden 키워드 보존 OK / 다른 단계는 신규 도입 X
