# GPT Pro 의뢰서 — spouse-01 재판관 어법 dispassionate sweep (2026-05-25)

## 0. 개요

신규 정책 **[[feedback_judge_dispassionate_action_focused]]** (Cycle 7 도입) 의 cross-case sweep 영역. spouse-01 의 재판관(judge) 발화 채널에서 위반 어휘 11 entries (KO baseline) polish 시안 요청.

- **사건**: 박지연·이준호 부부, 외도 의심 + 위임장 조작 + 공동 적금 해지 + 투자 사기방
- **영역**: judge_question (8) + judge_contradiction (3) = **11 entries**
- **위반 어휘 분포**: "선을 넘" 4 / "그 X" 7
...
```
spouse01-judge-dispassionate-sweep-20260525/
├── README.md (본 파일)
└── gpt-upload/
...
6. 적용 후 Codex multilang sync 의뢰서 별도 작성 (EN/JA/ZH-CN 11 × 3 = 33 entries)
  ...
  "polished": [
    {
      ...
      "new": "이준호 씨, 새벽마다 같은 번호와 짧게 통화하셨습니다. 관련 통화 상대를 말씀해 주시겠습니까.",
      "rationale": "지시 대명사 '그 통화' → 중립 '관련 통화'"
    },
    ...
  ]
}
...
- [[design_spouse01_truth_disclosure_policy]] — 진실 노출 정책 (변환 시 truth leak 회피)
- [[feedback_truth_leak_prohibition]] — 진실 누설 금지 핵심 원칙
- [[project_spouse01_event_timeline]] — 사건 흐름 timeline (변환 의미 검증)
- [[feedback_natural_korean_npc_active_voice]] / [[feedback_natural_korean_precision_guide]] — 자연 한국어 가이드

## 6. 자매 의뢰서

- `family01-judge-dispassionate-sweep-20260525/` — family-01 사건 14 entries 동일 정책 sweep
...
본 의뢰서는 spouse-01 Cycle 4 narrative wrapper 영역과 **분리 가능** (judge_question / judge_contradiction 채널은 dispute-stage 일관 영역이라 narrative wrapper의 emergence_narrative와 channel 충돌 X).
