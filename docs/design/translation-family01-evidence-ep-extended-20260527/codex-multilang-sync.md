# Codex Thread — family-01 evidence_present 확장 패턴 잔존 43 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/family01-ep-extended-sync ../ws-family01-ep-extended-sync main` |
...
| 산출 | branch `codex/family01-ep-extended-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/family-01.json`의 `evidence_present` channel **evidence_present 확장 패턴 잔존 43 entries** (commit on main: `66159b29`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 그_점 35 + 그_상황 8 단일 차원. 43 unique entries 1:1 polish.
- NPC 자기 진술 채널. "그 점/그 상황" → 구체 referent (당시의 일/구체 이유/기록에 남은 내용). 어머니/형 호명 영역 유지. 진실 누설 정책 부합 (출생/정후 돈/공장 영역 표면 보존).
...
- KO file — main에 이미 적용 완료 (commit `66159b29`)
- 다른 batch — 별도 의뢰서
...
main commit `66159b29`의 `git show 66159b29` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 66159b29 -- src/data/scriptedText/family-01.json
```
...
- worktree branch `codex/family01-ep-extended-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/family01-ep-extended-sync
  Commit: <hash>
