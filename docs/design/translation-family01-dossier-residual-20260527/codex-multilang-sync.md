# Codex Thread — family-01 dossier 어색 잔존 23 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/family01-dossier-residual-sync ../ws-family01-dossier-residual-sync main` |
...
| 산출 | branch `codex/family01-dossier-residual-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/family-01.json`의 `dossier` channel **dossier 어색 잔존 23 entries** (commit on main: `4c61c9b2`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 자기지시 자신 7 + 끝까지 5 + 완전히 4 + 그_판단 4 + 그렇게 4 + 그_점 1 다중 패턴. 23 unique entries 1:1 polish.
- NPC 1:1 답변 채널 (toJudge formal honorific). 자기지시 "자신" → 자연 자기 발화 ("저는/제가/제 ~"). 재판관님 호명 / 가족 호명 (형/어머니) 영역 유지.
...
- KO file — main에 이미 적용 완료 (commit `4c61c9b2`)
- 다른 batch — 별도 의뢰서
...
main commit `4c61c9b2`의 `git show 4c61c9b2` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 4c61c9b2 -- src/data/scriptedText/family-01.json
```
...
- worktree branch `codex/family01-dossier-residual-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/family01-dossier-residual-sync
  Commit: <hash>
