# Codex Thread — family-01 judge_* 채널 어색 잔존 56 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/family01-judge-residual-sync ../ws-family01-judge-residual-sync main` |
...
| 산출 | branch `codex/family01-judge-residual-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/family-01.json`의 **family-01 judge_* 채널 어색 잔존 56 entries** (commit on main: `42046270`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 자기지시:본인 + 차분히 + 직역어미 + 자기지시:자신 + 추상명사 다중 패턴. 56 unique entries 1:1 polish.
- 재판관 → NPC 발화 영역. judge_evidence_combo 28 + judge_question 14 + judge_contradiction 8 + judge_witness_summon 6 통합. 자기지시 "본인" only 정책 부합 영역은 batch 제외 (이미 보존됨). "자신" 9 영역 본인 통일. 가족 호명 (어머니 / 형) 보존.
- 채널: judge_evidence_combo / judge_question / judge_contradiction / judge_witness_summon
...
- KO file — main에 이미 적용 완료 (commit `42046270`)
- 다른 batch — 별도 의뢰서
...
main commit `42046270`의 `git show 42046270` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 42046270 -- src/data/scriptedText/family-01.json
```
...
judge_evidence_combo / judge_question / judge_contradiction / judge_witness_summon

### 3.2. 발화 주체 정책 (★ 핵심)

본 batch 발화 주체 관점:
- 재판관 → NPC 지칭에 "본인" 사용 자연 ([[design_self_reference_bonin_consistency]]) — 본 batch는 정책 부합 영역 보존
...
- worktree branch `codex/family01-judge-residual-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/family01-judge-residual-sync
  Commit: <hash>
