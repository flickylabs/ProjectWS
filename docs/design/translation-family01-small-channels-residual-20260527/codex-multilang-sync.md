# Codex Thread — family-01 small channel 어색 잔존 98 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/family01-small-residual-sync ../ws-family01-small-residual-sync main` |
...
| 산출 | branch `codex/family01-small-residual-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/family-01.json`의 **family-01 small channel 어색 잔존 98 entries** (commit on main: `b01f64cf`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 10개 채널 통합 (judge_* 제외). NPC 자기 발화 영역.
- 채널: interrogation 57 / aftermath 10 / rapport_milestone 6 / contradiction_pursuit 6 / witness 4 / trust_action 4 / evidence_discovery 4 / emotional_overload 3 / mediation 2 / contradict_milestone 2. NPC 자기지시 "본인/자신" → "저/제" 자연화 영역 (aftermath 자신 9 dominant). 가족 호명 보존.
- 채널: 10개 NPC 발화 채널
...
- KO file — main에 이미 적용 완료 (commit `b01f64cf`)
- 다른 batch — 별도 의뢰서
...
main commit `b01f64cf`의 `git show b01f64cf` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show b01f64cf -- src/data/scriptedText/family-01.json
```
...
- worktree branch `codex/family01-small-residual-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/family01-small-residual-sync
  Commit: <hash>
