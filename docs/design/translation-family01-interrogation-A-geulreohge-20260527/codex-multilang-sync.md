# Codex Thread — family-01 interrogation Batch A "그렇게" 109 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/family01-interrogation-A-sync ../ws-family01-interrogation-A-sync main` |
...
| 산출 | branch `codex/family01-interrogation-A-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/family-01.json`의 `interrogation` channel **109 entries** (commit on main: `8e99c402`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 강조부사 "그렇게" 단일 패턴. 109 unique entries 1:1 polish. family-01 사건 (어머니 유산 가족 분쟁).
- NPC 자기 진술 channel. lieState 단계 톤 보존. hidden keyword 신규 도입 X.
...
- KO file — main에 이미 적용 완료 (commit `8e99c402`)
- 다른 batch — 별도 의뢰서
...
main commit `8e99c402`의 `git show 8e99c402` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 8e99c402 -- src/data/scriptedText/family-01.json
```
...
- worktree branch `codex/family01-interrogation-A-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/family01-interrogation-A-sync
  Commit: <hash>
