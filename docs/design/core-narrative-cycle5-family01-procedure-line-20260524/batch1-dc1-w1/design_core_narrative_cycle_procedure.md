...
- **`npm run build` PASS 필수** (tsc -b --force + vite build). `tsc --noEmit`만 통과해도 project references build에서 잡힐 수 있음 (Cycle 1 사후 통합 시점 발견 — `narrativeOrchestrator.ts` import 경로 TS2305 에러)
- **`npm run -s qa:fast` PASS 필수 — RELEASE READY 확인 (static P0=0, route P0=0)**
  - Cycle 2 시점 사고: build PASS만 보고 push → Codex 4 thread가 외국어 작업 후 qa:fast P0 5건 (`emerge-e6/dc-4` surface-name gate 위반) 보고 → 사후 통합에서 KO + 외국어 모두 fix 필요했음. build와 qa:fast는 측정 차원이 다름 — qa:fast는 ScriptedText surface-name gate / truth-leak gate 등 게임 정책 위반 검출
  - 발견 시 `tmp/qa-runtime-gate-results/findings.json` 의 `severity:"P0"` filter로 정확한 위치/메시지 식별 후 fix
...
- 통합 방식 선택:
  - **fast-forward merge** — 단일 batch 또는 변경 영역이 다른 file일 때
  - **JSON union script** — 여러 batch가 같은 file의 같은 배열에 entry 추가하는 경우. `git merge`는 JSON 배열 영역에서 충돌 발생 (Cycle 2 시점 사고). node script로 `channels.X.entries` 배열에 새 key entry만 union append
- **최종 `npm run build` PASS 필수** (Cycle 1 시점 `tsc --noEmit` 만 통과 후 push했다가 build에서 잡혀 hotfix commit 추가 발생)
- **최종 `npm run -s qa:fast` PASS 필수** (RELEASE READY). 통합 후 외국어 file 영역에도 KO와 동일 P0가 있을 수 있음 → 동일 패턴 fix
- worktree 정리 (`git worktree remove --force ../ws-...`) — 사용자가 IDE 창을 닫지 않으면 Windows file lock으로 permission denied. 사용자에게 IDE 창 닫기 요청 필수
