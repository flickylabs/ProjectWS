# Codex Thread — spouse-01 interjection 어색 잔존 24 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/spouse01-interjection-residual-sync ../ws-spouse01-interjection-residual-sync main` |
...
| 산출 | branch `codex/spouse01-interjection-residual-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/spouse-01.json`의 `interjection` channel **interjection 어색 잔존 24 entries** (commit on main: `baa06777`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 그렇게 17 + 그_점 4 + 끝까지 4 + 그_판단 1 다중 패턴. 24 unique entries 1:1 polish.
- NPC 끼어듦 채널. major/minor interjection 톤 보존 (항의/거부/방어/감정 호소). 호명 영역 (이준호 / 박지연 직접 호명) 보존.
...
- KO file — main에 이미 적용 완료 (commit `baa06777`)
- 다른 batch — 별도 의뢰서
...
main commit `baa06777`의 `git show baa06777` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show baa06777 -- src/data/scriptedText/spouse-01.json
```
...
- worktree branch `codex/spouse01-interjection-residual-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/spouse01-interjection-residual-sync
  Commit: <hash>
