# Codex Thread — friend-01 judge_evidence_combo 어색 잔존 22 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/friend01-jec-sync ../ws-friend01-jec-sync main` |
...
| 산출 | branch `codex/friend01-jec-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/friend-01.json`의 `judge_evidence_combo` channel **judge_evidence_combo 어색 잔존 22 entries** (commit on main: `58b131ea`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 차분히 6 + 이어집니다 5 + 끝까지 4 + 가리킵니다 3 + 그_결론 2 + 그_판단 2 + 그렇게 1 + 드러납니다 1 다중 패턴. 22 unique entries 1:1 polish.
- 재판관 → NPC 추궁 채널 (judge speaker). 재판관 → NPC "본인" 지칭 영역은 [[design_self_reference_bonin_consistency]] 정책 부합 영역으로 보존. friend-01 hidden keyword 신규 도입 X.
...
- KO file — main에 이미 적용 완료 (commit `58b131ea`)
- 다른 batch — 별도 의뢰서
...
main commit `58b131ea`의 `git show 58b131ea` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show 58b131ea -- src/data/scriptedText/friend-01.json
```
...
재판관이 evidence 조합을 NPC에 제시·추궁하는 채널. soft/mid/hard 3단계 청유 어미.

### 3.2. 발화 주체 정책 (★ 핵심)

본 batch 발화 주체 관점:
- 재판관 → NPC 지칭에 "본인" 사용 자연 ([[design_self_reference_bonin_consistency]]). 본 batch 정책 부합 영역 보존.
...
|---------|-----------|----------------|
| 강조 부사 완화 | "그렇게/끝까지/차분히/완전히/진짜" → 명확 표현 또는 제거 | EN 단정 어휘 완화 / JA 副詞 자연화 / ZH-CN 정도 부사 자연화 |
| 추상 referent 구체화 | "그 부분/그 점/그 판단/그 결론/그 상황" → 구체 referent | 구체 referent 다국어 적용 |
| 직역체 자연화 | "가리킵니다 / 드러납니다 / 이어집니다" → "맞물려 / 확인됩니다 / 연결됩니다" | 어미 자연화 |
| 자기지시 정리 | NPC 자기 발화 영역 "본인/자신" → "저/제" | EN "I/me" / JA "私" / ZH-CN "我" 일관 |
...
4. `node scripts/verify-translations.cjs --strict --scan-applied` (현재 baseline ~57,977 — 회귀 X 확인, sync 후 감소 기대)
...
- worktree branch `codex/friend01-jec-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/friend01-jec-sync
  Commit: <hash>
  검증: tsc/qa:fast/truth-leak/verify-translations PASS (total <숫자>)
  변경: friend-01.en/ja/zh-CN.json (... entries)
  ```
