# Codex Thread — spouse-01 evidence_present 확장 패턴 잔존 47 entries 다국어 sync
...
| 항목 | 조건 |
|---|---|
| **worktree spawn** | `git worktree add -b codex/spouse01-ep-extended-sync ../ws-spouse01-ep-extended-sync main` |
...
| 산출 | branch `codex/spouse01-ep-extended-sync` push |

---

## §1. 작업 범위

KO `src/data/scriptedText/spouse-01.json`의 `evidence_present` channel **evidence_present 확장 패턴 잔존 47 entries** (commit on main: `a088c961`). EN/JA/ZH-CN 동일 anchor 동기.

본 batch 특이성:
- 그_부분 16 + 그렇게 12 + 그_점 7 + 드러납니다 6 + 완전히 6 다중 패턴. 47 unique entries 1:1 polish.
- NPC 자기 진술 채널. "그 부분/그 점" → 구체 referent (자료 내용/자금 흐름 등). "드러납니다" → "알려질 수밖에 없습니다". 가족 직접 언급 → "다른 관련자" 회피 (진실 누설 정책 부합).
...
- KO file — main에 이미 적용 완료 (commit `a088c961`)
- 다른 batch — 별도 의뢰서
...
main commit `a088c961`의 `git show a088c961` 결과로 KO diff 확인. 각 변경 entry는 ID 기준으로 EN/JA/ZH-CN file의 동일 ID entry sync.

명령 예시:
```
git show a088c961 -- src/data/scriptedText/spouse-01.json
```
...
### 3.1. evidence_present 채널 특이성

NPC가 증거를 마주하고 자기 진술하는 채널. archetype 일관성 (같은 KO → 같은 외국어).
...
- worktree branch `codex/spouse01-ep-extended-sync` push
- main session에서 cherry-pick 후 cleanup
- 결과 보고 양식:
  ```
  완료. Branch: codex/spouse01-ep-extended-sync
  Commit: <hash>
