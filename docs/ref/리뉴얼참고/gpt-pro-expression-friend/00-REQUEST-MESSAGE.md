# GPT Pro 요청: friend-01 ScriptedText 표현 개선

## 목적
friend-01(손절한 절친) ScriptedText 번들의 **감정 깊이와 behaviorHint 품질**을 spouse-01 수준으로 개선.
...
**Step 2**: `friend-01-scriptedtext-current.json`에서 **interrogation 채널(144 entries)**만 먼저 개선하세요.
...
`friend-01-scriptedtext-improved.json`
...
| 파일 | 용도 |
|------|------|
| `friend-01-scriptedtext-current.json` | **작업 대상** — 현재 ScriptedText 번들 전체 |
| `reference-spouse-01-quality-samples.json` | **품질 기준** — spouse-01에서 추출한 채널별 샘플 (개선 완료 상태) |
| `ref-friend-story.md` | 스토리 설계 (쟁점/증거/LieState/캐릭터/증인) |

## 개선 기준

### behaviorHint 개선 (최우선)
**Before** (현재, 약함):
> "잠깐 멈칫했다가 낮은 목소리로 말한다."
...
### 캐릭터 톤
- **A 송다은** (premature_summary): 단정적, 성급한 결론. "예전에도 이래서 손절했다", "또 그런 거잖아"
- **B 최수민** (affect_flattening): 감정 억제, 말끝이 흐려짐. "... 그런 뜻은 아니었습니다.", "... 말하기가 어렵습니다."
