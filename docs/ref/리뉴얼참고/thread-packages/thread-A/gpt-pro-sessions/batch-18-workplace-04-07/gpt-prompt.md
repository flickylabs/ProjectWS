# GPT Pro 세션 — 배치 18: workplace-04~07 investigationStages 생성
...
| `workplace-04-input.json` | case-work-04 사건 데이터 |
| `workplace-05-input.json` | case-work-05 사건 데이터 |
| `workplace-06-input.json` | case-work-06 사건 데이터 |
| `workplace-07-input.json` | case-work-08 사건 데이터 |
...
첨부된 4건의 사건 JSON(workplace-04, workplace-05, workplace-06, workplace-07)을 읽고, 모든 증거에 대해 3단계 investigationStages를 생성해.
...
- case-work-04: 마감 실패는 민석의 18시 승인 약속 불이행과 지우의 회귀 테스트 미완료 상태 초록불 표시가 겹친 공동 책임이다.
- case-work-05: 가은의 '순수 창작' 고객회복 랩의 핵심 구조는 2년 전 컨설턴트 배수진의 보류 파일에서 왔고, 상혁은 그걸 알면서 공동기획 표기를 지웠다.
- case-work-06: 통합 공지는 어느 쪽에도 단독 통제권을 주지 않았다. 첫 2주간 계정 변경·규칙 수정은 공동 승인+PM 기록이 필수였다.
- case-work-08: 세린이 올린 '증거인멸 지시' 캡처의 본문은 2년 전 품질등급 축소 채팅이고, 헤더만 현재 프로젝트 채널로 바꿔 붙인 합성본이었다.
