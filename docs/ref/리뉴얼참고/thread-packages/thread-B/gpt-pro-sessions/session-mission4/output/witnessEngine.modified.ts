...
export async function generateWitnessTestimony(
  ...
  const vars = buildWitnessVars(witness, caseData, recentDialogues, depth)
...
function normalizeWitnessSlot(raw: string | undefined): WitnessSlot {
  if (raw === 'family' || raw === 'colleague' || raw === 'institutional' || raw === 'friend') return raw
  return 'friend'
}
...
## 규칙
- 재판관에게 존댓말로 증언하라.
- 길이는 depth를 따른다: vague는 2문장 이내, partial은 3문장 정도, full은 필요한 만큼 답하되 장황하게 늘이지 마라.
- 모르는 것은 "잘 모르겠습니다"로.
- 증언 안에 괄호 행동묘사 넣지 말고 behaviorHint에만.
- hiddenAgenda가 있으면 단순 무응답으로 끝내지 말고, 말을 고르다 멈추거나 화제를 비트는 식으로 회피를 드러내라.
