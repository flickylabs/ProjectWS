...
송다은 추가 규칙:
- 기본 프레임은 "수민이가 또 선을 넘었다"는 성급한 결론이다
- A는 premature_summary이므로 몇 장면만 보고 과거와 현재를 한꺼번에 묶어 말하는 습관이 살아 있어야 한다
- d-1 early에서는 집착 프레임이 가장 강해야 한다
- d-4에서는 예전 손절까지 같은 문제 인물 서사로 묶는 요약이 강해야 한다
- d-5에서는 단톡방 공개를 자기방어처럼 정당화하되 late에는 직접 확인보다 낙인을 먼저 택한 책임이 보여야 한다
- h-d2와 h-d3 late에서는 "내가 몰랐던 것"이 열리며 불안과 수치가 같이 올라와야 한다
...
- friend v3는 발췌본만 보면 B가 나빠 보이고, 맥락이 복원될수록 A의 낙인과 B의 침묵이 같이 재평가되는 구조다

Lead 해석 반영 규칙:
- L-1은 집착처럼 보이는 그림 vs 설명 빠진 경고
- L-2는 예전에도 그랬다 vs 예전에도 공백이 있었다
- L-3는 B의 핑계 vs A 아버지의 반복 패턴
- L-4는 말을 비틀어 오해를 키움 vs 친구가 무너질까 봐 직접 못 말함
- L-5는 친구 집착 사건을 두 번 반복된 같은 패턴으로 재정의한다

validator 절대 규칙:
- evidence_present와 dossier는 모든 variant가 `재판관님,`으로 시작
- evidence_present는 해당 증거 키워드를 반드시 직접 포함
- d-1 계열은 직접 금전어 금지
- friend-v3-01은 monetary dispute가 없으므로 금액 의무 노출 규칙은 적용하지 않는다
...
증인 규칙:
- w-1 김세라는 단톡방 낙인 흐름과 A 쪽 정서를 증언한다
- w-2 박준혁은 예비신랑의 분위기와 메시지 맥락을 증언한다
- w-3 오미경은 과거 돈 대화와 B의 무너짐을 증언한다
...
## 잔여 메모
- `resultClasses`, `system_keys`는 friend-v3-01 runtime/helper에 사건별 override가 없어서 current writer bundle baseline을 그대로 사용했다.
- 그 외 caseId, 쟁점, 증거, dossier, witness, authority, official record, hidden dispute unlock 구조는 모두 friend-v3-01 실데이터로 채웠다.
