# GPT Pro 의뢰 패키지 — 사용 가이드 (spouse-01 ScriptedText 보완)

> **작성**: 2026-04-26 (메인 Claude Opus 4.7)
> **목적**: spouse-01 ScriptedText (현재 1,253 entries) → 보완 후 ≈ 4,503 entries
...
│   ├── 01-character-info.md           # 박지연/이준호 + 증인 3종
│   ├── 02-tone-guide.md               # 보정 톤 8원칙
│   ├── 03-phase1-2-dialogue.md        # base voice 톤 참고
│   ├── 04-case-spouse-01.json         # 사건 정의 (evidences/dossier 포함)
...
**1단계: 프로젝트 셋업 (1회만)**
1. GPT Pro에서 **새 프로젝트 생성** (예: "Solomon Court — spouse-01 ScriptedText 보완")
...
3. `src/data/scriptedText/spouse-01.json` patch 적용
...
- **활성 3건만**: spouse-01 / family-01 / friend-01 — `_LEGACY_*` 절대 참조 X
...
→ 이 13 patch는 이미 `src/data/scriptedText/spouse-01.json`에 적용됨 (`10-scripted-text-current.json`에 반영). GPT Pro는 이 보정본을 base로 신규 작성.
...
## 후속 단계 (spouse-01 완료 후)

1. 8 세션 모두 완료 → spouse-01 ScriptedText 보완 종결
2. **family-01 / friend-01 동일 절차 반복** (각각 별도 GPT Pro 프로젝트 생성)
   - 단 세션 패턴은 spouse-01과 동일 → prompt.md 템플릿 재사용 가능
