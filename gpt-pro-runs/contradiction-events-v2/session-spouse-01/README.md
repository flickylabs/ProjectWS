# spouse-01 V2 데이터 생성 — GPT Pro 작업 지시
...
| `02-spec-spouse-01.yaml` | 사건별 스펙 + truth 보호 목록 + 톤 가이드 |
...
- `id` 패턴: `spouse-01-{disputeId}-{party}-{lieState}-c{n}` (예: `spouse-01-d1-a-S2-c1`)
...
2. **truth 보호**: `02-spec-spouse-01.yaml`의 `banned_lexemes_safe` 단어(`위임장`, `조작`, `사기`, `개인회생`, `조카`, `형`, `비자금`)가 `safe` 등급 어디든 등장하면 R2 ERROR.
3. **호명**: `judgePursuitLine`은 `${name}` (`박지연`/`이준호`) 또는 `callTerms.toJudge` (`제 남편`/`제 아내`) 포함 필수.
...
- **NPC (defenderReactionLine, partyA/B.statement)**:
  - 박지연 (archetype: `victim_cosplay`) — 감정형, 피해자 자기연출
  - 이준호 (archetype: `avoidant`) — 얼버무림, 회피
...
`02-spec-spouse-01.yaml`의 `example_safe_contradiction` 섹션 참조 (톤·구조 견본).
