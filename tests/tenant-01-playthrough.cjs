/**
 * tenant-01 V2 풀 플레이스루 테스트 — 심문 + 증거 포함 20턴
 * 세입자-집주인 관계 사건: 윤하림(partyA, avoidant) vs 최상우(partyB, cold_logic)
 * 보증금 정산·원상복구 갈등 — 5개 쟁점(d-1~d-5), S1~S5, 4종 질문, 증거 제시
 */
const fs = require('fs')
...
const v2Data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'docs/ref/리뉴얼참고/gpt-batch/tenant-01/tenant-01-v2-atoms.json'), 'utf8'))
const caseData = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'src/data/cases/generated/tenant-01.json'), 'utf8'))
...
async function run(label, party, disputeId, state, subAction, judgeQ, evidenceInfo) {
  ...
  try {
    ...
    // 정확한 금액 노출 (보증금 관련)
    if (resp.match(/5[,.]?00만/) && subAction !== 'evidence_present' && stance !== 'confess') issues.push('exact금액_보증금')
    // 반말 종결 / 되묻기
    if (resp.match(/[해야겠어궁금해생각해어떻게]\s*[?？]?\s*$/)) issues.push('반말종결/되묻기')
    // 이름 직접 호칭 (재판관 앞에서)
    if (party === 'a' && resp.match(/상우[가을를은는씨]/) && !resp.includes('집주인')) issues.push('이름직접호칭_A')
    if (party === 'b' && resp.match(/하림[이가을를은는씨]/) && !resp.includes('세입자')) issues.push('이름직접호칭_B')
    // 존댓말 일관성: 반말 종결어미 탐지
...
async function main() {
  console.log('===================================================================')
  console.log('  tenant-01 V2 풀 플레이스루 (심문 + 증거) — 20턴')
  console.log(`  사건: 보증금 정산·원상복구 갈등 (윤하림 vs 최상우)`)
  ...
  // ═══════════════════════════════════════════
  // 1~3턴: 윤하림 d-1 초기 심문 (S1)
  // ═══════════════════════════════════════════
  console.log('--- 1~3턴: 윤하림 d-1 S1 초기 심문 ---')
  await run('윤하림 d-1 S1 사실추궁', 'a', 'd-1', 'S1', 'fact_pursuit',
    '윤하림 씨, 마지막 월세를 5일 늦게 납부한 것이 사실입니까?')
  await delay(2000)
  await run('윤하림 d-1 S1 동기탐색', 'a', 'd-1', 'S1', 'motive_search',
    '윤하림 씨, 월세 납부가 늦어진 이유가 무엇입니까?')
  await delay(2000)
  await run('윤하림 d-1 S1 공감접근', 'a', 'd-1', 'S1', 'empathy_approach',
    '윤하림 씨, 이사를 앞두고 보증금 문제로 어떤 심정이셨습니까?')
  await delay(2000)

  // ═══════════════════════════════════════════
  // 4~5턴: 최상우 d-1/d-2 초기 심문 (S1)
  // ═══════════════════════════════════════════
  console.log('--- 4~5턴: 최상우 d-1/d-2 S1 초기 심문 ---')
  await run('최상우 d-1 S1 사실추궁', 'b', 'd-1', 'S1', 'fact_pursuit',
    '최상우 씨, 세입자의 월세 지연 납부 사실을 확인하겠습니다.')
  await delay(2000)
  await run('최상우 d-2 S1 사실추궁', 'b', 'd-2', 'S1', 'fact_pursuit',
    '최상우 씨, 거실 벽지 전면 교체가 필요하다고 판단하신 근거가 무엇입니까?')
  await delay(2000)
  ...
  await run('윤하림 e-1 증거제시', 'a', 'd-2', 'S1', 'evidence_present',
    '윤하림 씨, 이 입주·퇴거 비교 사진에 대해 입장을 밝혀 주십시오.',
    `증거: ${e1.name}\n설명: ${e1.description.slice(0, 120)}`)
  await delay(2000)
  await run('최상우 e-1 증거제시', 'b', 'd-2', 'S1', 'evidence_present',
    '최상우 씨, 이 입주·퇴거 비교 사진을 보시고 어떻게 생각하십니까?',
    `증거: ${e1.name}\n설명: ${e1.description.slice(0, 120)}`)
  await delay(2000)

  // ═══════════════════════════════════════════
  // 8~9턴: 윤하림 d-1 S2 (부분 인정)
  // ═══════════════════════════════════════════
  console.log('--- 8~9턴: 윤하림 d-1 S2 부분 인정 ---')
  await run('윤하림 d-1 S2 사실추궁', 'a', 'd-1', 'S2', 'fact_pursuit',
    '윤하림 씨, 월세 지연 경위에 대해 좀 더 구체적으로 말씀해 주십시오.')
  await delay(2000)
  await run('윤하림 d-2 S2 동기탐색', 'a', 'd-2', 'S2', 'motive_search',
    '윤하림 씨, 반려묘로 인한 벽지 손상을 왜 미리 말씀하지 않으셨습니까?')
  await delay(2000)

  // ═══════════════════════════════════════════
  // 10~11턴: 최상우 d-2 S2
  // ═══════════════════════════════════════════
  console.log('--- 10~11턴: 최상우 d-2 S2 ---')
  await run('최상우 d-2 S2 사실추궁', 'b', 'd-2', 'S2', 'fact_pursuit',
    '최상우 씨, 전면 교체 견적을 받으신 경위를 구체적으로 말씀해 주십시오.')
  await delay(2000)
  await run('최상우 d-2 S2 공감접근', 'b', 'd-2', 'S2', 'empathy_approach',
    '최상우 씨, 세입자가 집을 손상시켰다고 느꼈을 때 어떤 심정이셨습니까?')
  await delay(2000)
  ...
  await run('윤하림 e-2 증거제시', 'a', 'd-1', 'S2', 'evidence_present',
    '윤하림 씨, 이 월세·관리비 입금 내역에 대해 입장을 밝혀 주십시오.',
    `증거: ${e2.name}\n설명: ${e2.description.slice(0, 120)}`)
  await delay(2000)
  await run('최상우 e-3 증거제시', 'b', 'd-4', 'S1', 'evidence_present',
    '최상우 씨, 이 카카오톡 캡처와 통화 녹취 요약에 대해 설명해 주십시오.',
    `증거: ${e3.name}\n설명: ${e3.description.slice(0, 120)}`)
  await delay(2000)

  // ═══════════════════════════════════════════
  // 14~15턴: 윤하림 d-2 S3 (전가/반격)
  // ═══════════════════════════════════════════
  console.log('--- 14~15턴: 윤하림 d-2 S3 ---')
  await run('윤하림 d-2 S3 사실추궁(blame)', 'a', 'd-2', 'S3', 'fact_pursuit',
    '윤하림 씨, 벽지 손상 범위에 대해 사실관계가 분명하지 않습니다. 다시 설명해 주십시오.')
  await delay(2000)
  await run('윤하림 d-3 S1 사실추궁', 'a', 'd-3', 'S1', 'fact_pursuit',
    '윤하림 씨, 욕실 수전 누수에 대해 집주인분께 언제 말씀하셨습니까?')
  await delay(2000)

  // ═══════════════════════════════════════════
  // 16~17턴: 최상우 d-2/d-3 S3
  // ═══════════════════════════════════════════
  console.log('--- 16~17턴: 최상우 d-2/d-5 S3 ---')
  await run('최상우 d-2 S3 사실추궁', 'b', 'd-2', 'S3', 'fact_pursuit',
    '최상우 씨, 벽지 교체 범위에 대해 다시 정리해 주십시오.')
  await delay(2000)
  await run('최상우 d-5 S1 사실추궁', 'b', 'd-5', 'S1', 'fact_pursuit',
    '최상우 씨, 전면 공제를 통보하신 범위가 계약 특약 기준에 맞는지 설명해 주십시오.')
  await delay(2000)

  // ═══════════════════════════════════════════
  // 18~19턴: 윤하림 S4 (감정), 증거 e-4/e-5
  // ═══════════════════════════════════════════
  console.log('--- 18~19턴: 윤하림 S4 + 증거 ---')
  await run('윤하림 d-2 S4 공감접근(emotional)', 'a', 'd-2', 'S4', 'empathy_approach',
    '윤하림 씨, 당시 심정을 충분히 듣겠습니다. 솔직하게 말씀해 주십시오.')
  await delay(2000)
  await run('윤하림 e-4 증거제시', 'a', 'd-2', 'S2', 'evidence_present',
    '윤하림 씨, 이 벽지 부분보수·전면도배 비교 견적서에 대해 입장을 밝혀 주십시오.',
  ...
  // 20턴: 윤하림 S5 (고백)
  // ═══════════════════════════════════════════
  console.log('--- 20턴: 윤하림 d-1 S5 고백 ---')
  await run('윤하림 d-1 S5 사실추궁(confess)', 'a', 'd-1', 'S5', 'fact_pursuit',
    '윤하림 씨, 이제 사실대로 말씀해 주십시오.')
  ...
  for (const r of results.filter(r => !r.error)) {
    ...
    const hasTenantTerms = /집주인|세입자|임대인|임차인/.test(resp)
    const honorific = !/[해야겠어궁금해알아몰라싶어거야건데거든]$/.test(resp.replace(/\s+/g, ''))
    console.log(`  #${r.num} ${r.label}: wrong=${hasWrongTerms ? 'LEAK!' : 'ok'} tenant=${hasTenantTerms ? 'ok' : '-'} honorific=${honorific ? 'ok' : 'WARN'}`)
  }

  // 상태 진행 요약
  console.log('\n--- 윤하림 d-1/d-2 상태 진행 ---')
  for (const r of results.filter(r => r.party === 'a' && !r.error)) {
    console.log(`  ${r.disputeId} ${r.state} [${r.subAction}${r.evidence ? '+증거' : ''}]: ${r.response?.slice(0, 80)}`)
  }
  console.log('\n--- 최상우 d-2 상태 진행 ---')
  ...
  // 결과 저장
  const outPath = path.join(__dirname, '..', 'docs/ref/리뉴얼참고/thread-packages/thread-E/reports/tenant-01-playthrough-results.txt')
