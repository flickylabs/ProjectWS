...
async function run(label, party, disputeId, state, subAction, judgeQ, evidenceInfo, caseData, v2Data, ctx) {
  ...
  if (!policy?.claimAtoms) { console.log(`  [SKIP] #${ctx.turnNum} ${label}: atoms 없음`); return null }
  ...
  const tellReminder = ''  // tell 리마인더는 시스템 프롬프트에서 충분
...
function generateReport(results, caseData) {
  ...
  for (const r of results.filter(r => !r.error)) {
    ...
    console.log(`  #${r.num} ${r.label}: honorific=${honorific ? 'ok' : 'WARN'}`)
  }

  // 상태 진행 요약 (partyA 주요 쟁점)
  const partyAName = caseData.duo.partyA.name
  const partyBName = caseData.duo.partyB.name
  const aResults = results.filter(r => r.party === 'a' && !r.error)
  const bResults = results.filter(r => r.party === 'b' && !r.error)

  if (aResults.length > 0) {
    const aDisputes = [...new Set(aResults.map(r => r.disputeId))].sort()
    for (const d of aDisputes) {
      console.log(`\n--- ${partyAName} ${d} 상태 진행 ---`)
      for (const r of aResults.filter(r => r.disputeId === d)) {
        console.log(`  ${r.state} [${r.subAction}${r.evidence ? '+증거' : ''}]: ${r.response?.slice(0, 80)}`)
      }
    }
  }
  if (bResults.length > 0) {
    const bDisputes = [...new Set(bResults.map(r => r.disputeId))].sort()
    for (const d of bDisputes) {
      console.log(`\n--- ${partyBName} ${d} 상태 진행 ---`)
      for (const r of bResults.filter(r => r.disputeId === d)) {
        console.log(`  ${r.state} [${r.subAction}${r.evidence ? '+증거' : ''}]: ${r.response?.slice(0, 80)}`)
      }
    }
  }
