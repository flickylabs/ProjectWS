/**
 * neighbor-01 V2 풀 플레이스루 테스트 — 심문 + 증거 포함 20턴
 * 이웃 관계 사건: 정한결(partyA, confrontational) vs 이주연(partyB, avoidant)
 * 누수·촬영·소음 갈등 — 5개 쟁점(d-1~d-5), S1~S5, 4종 질문, 증거 제시
 */
const fs = require('fs')
...
const v2Data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'docs/ref/리뉴얼참고/gpt-batch/neighbor-01/neighbor-01-v2-atoms.json'), 'utf8'))
const caseData = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'src/data/cases/generated/neighbor-01.json'), 'utf8'))
...
const ARCHETYPE = { avoidant: '회피형: 화제 전환, 순서/절차 앞세움. 말이 길어짐.', confrontational: '정면돌파형: 짧고 날카로움. 역공 선호.', victim_cosplay: '피해자 행세형: 희생 목록을 먼저 늘어놓아 방어.', cold_logic: '냉정논리형: 숫자와 절차로 감정을 차단.' }
const FOCUS = {
...
async function run(label, party, disputeId, state, subAction, judgeQ, evidenceInfo) {
  ...
  try {
    ...
    // 정확한 시각 노출 (0시 48분 — anchorTruth 관련)
    if (resp.match(/0시\s*48분/) && subAction !== 'evidence_present') issues.push('exact시각_0시48분')
    // 반말 종결 / 되묻기
    if (resp.match(/[해야겠어궁금해생각해어떻게]\s*[?？]?\s*$/)) issues.push('반말종결/되묻기')
    // 이름 직접 호칭 (재판관 앞에서)
    if (party === 'a' && resp.match(/주연[이가을를은는씨]/) && !resp.includes('옆집')) issues.push('이름직접호칭_A')
    if (party === 'b' && resp.match(/한결[이가을를은는씨]/) && !resp.includes('옆집')) issues.push('이름직접호칭_B')
    // 존댓말 일관성: 반말 종결어미 탐지
...
async function main() {
  console.log('===================================================================')
  console.log('  neighbor-01 V2 풀 플레이스루 (심문 + 증거) — 20턴')
  console.log(`  사건: 누수·촬영·소음 갈등 (정한결 vs 이주연)`)
  ...
  // ═══════════════════════════════════════════
  // 1~3턴: 정한결 d-1 초기 심문 (S1)
  // ═══════════════════════════════════════════
  console.log('--- 1~3턴: 정한결 d-1 S1 초기 심문 ---')
  await run('정한결 d-1 S1 사실추궁', 'a', 'd-1', 'S1', 'fact_pursuit',
    '정한결 씨, 504호 현관 앞에서 촬영한 것이 사실입니까?')
  await delay(2000)
  await run('정한결 d-1 S1 동기탐색', 'a', 'd-1', 'S1', 'motive_search',
    '정한결 씨, 새벽에 복도에서 촬영한 이유가 무엇입니까?')
  await delay(2000)
  await run('정한결 d-1 S1 공감접근', 'a', 'd-1', 'S1', 'empathy_approach',
    '정한결 씨, 딸이 새벽에 물소리에 깨는 상황에서 어떤 심정이셨습니까?')
  await delay(2000)

  // ═══════════════════════════════════════════
  // 4~5턴: 이주연 d-1/d-2 초기 심문 (S1)
  // ═══════════════════════════════════════════
  console.log('--- 4~5턴: 이주연 d-1/d-2 S1 초기 심문 ---')
  await run('이주연 d-1 S1 사실추궁', 'b', 'd-1', 'S1', 'fact_pursuit',
    '이주연 씨, 아래층에서 본인 현관을 촬영했다는 사실을 알고 계셨습니까?')
  await delay(2000)
  await run('이주연 d-2 S1 사실추궁', 'b', 'd-2', 'S1', 'fact_pursuit',
    '이주연 씨, 제습기와 물받이를 복도에 48시간 동안 두었다는 것이 사실입니까?')
  await delay(2000)
  ...
  await run('정한결 e-1 증거제시', 'a', 'd-1', 'S1', 'evidence_present',
    '정한결 씨, 이 새벽 복도 촬영 영상 캡처에 대해 입장을 밝혀 주십시오.',
    `증거: ${e1.name}\n설명: ${e1.description?.slice(0, 120) ?? '새벽 복도 촬영 캡처'}`)
  await delay(2000)
  await run('이주연 e-1 증거제시', 'b', 'd-1', 'S1', 'evidence_present',
    '이주연 씨, 이 촬영 캡처를 보시고 어떻게 생각하십니까?',
    `증거: ${e1.name}\n설명: ${e1.description?.slice(0, 120) ?? '새벽 복도 촬영 캡처'}`)
  await delay(2000)

  // ═══════════════════════════════════════════
  // 8~9턴: 정한결 d-1 S2 (부분 인정)
  // ═══════════════════════════════════════════
  console.log('--- 8~9턴: 정한결 d-1 S2 부분 인정 ---')
  await run('정한결 d-1 S2 사실추궁', 'a', 'd-1', 'S2', 'fact_pursuit',
    '정한결 씨, 촬영 횟수와 범위에 대해 좀 더 구체적으로 말씀해 주십시오.')
  await delay(2000)
  await run('정한결 d-1 S2 동기탐색', 'a', 'd-1', 'S2', 'motive_search',
    '정한결 씨, 관리사무소에 먼저 연락하지 않고 직접 촬영한 이유가 무엇입니까?')
  await delay(2000)

  // ═══════════════════════════════════════════
  // 10~11턴: 이주연 d-2 S2
  // ═══════════════════════════════════════════
  console.log('--- 10~11턴: 이주연 d-2 S2 ---')
  await run('이주연 d-2 S2 사실추궁', 'b', 'd-2', 'S2', 'fact_pursuit',
    '이주연 씨, 아래층에 연락을 31시간 늦게 보낸 경위를 구체적으로 말씀해 주십시오.')
  await delay(2000)
  await run('이주연 d-2 S2 공감접근', 'b', 'd-2', 'S2', 'empathy_approach',
    '이주연 씨, 복도에 물건을 두고 아래층 분에게 연락이 늦어진 것에 대해 어떤 심정이셨습니까?')
  await delay(2000)
  ...
  await run('정한결 e-2 증거제시', 'a', 'd-3', 'S1', 'evidence_present',
    '정한결 씨, 이 누수 점검 보고서에 대해 입장을 밝혀 주십시오.',
    `증거: ${e2.name}\n설명: ${e2.description?.slice(0, 120) ?? '4라인 누수 점검 보고서'}`)
  await delay(2000)
  await run('이주연 e-3 증거제시', 'b', 'd-2', 'S2', 'evidence_present',
    '이주연 씨, 이 CCTV 기록과 출입대장에 대해 설명해 주십시오.',
    `증거: ${e3.name}\n설명: ${e3.description?.slice(0, 120) ?? '공용복도 CCTV와 출입대장'}`)
  await delay(2000)

  // ═══════════════════════════════════════════
  // 14~15턴: 정한결 d-1 S3 (전가/반격)
  // ═══════════════════════════════════════════
  console.log('--- 14~15턴: 정한결 d-1 S3 ---')
  await run('정한결 d-1 S3 사실추궁(blame)', 'a', 'd-1', 'S3', 'fact_pursuit',
    '정한결 씨, 촬영 건에 대해 사실관계가 분명하지 않습니다. 다시 설명해 주십시오.')
  await delay(2000)
  await run('정한결 d-3 S1 사실추궁', 'a', 'd-3', 'S1', 'fact_pursuit',
    '정한결 씨, 누수 원인이 윗집이라고 판단하신 근거가 무엇입니까?')
  await delay(2000)

  // ═══════════════════════════════════════════
  // 16~17턴: 이주연 d-2/d-3 S3
  // ═══════════════════════════════════════════
  console.log('--- 16~17턴: 이주연 d-2/d-3 S3 ---')
  await run('이주연 d-2 S3 사실추궁', 'b', 'd-2', 'S3', 'fact_pursuit',
    '이주연 씨, 복도 적치와 연락 지연에 대해 다시 정리해 주십시오.')
  await delay(2000)
  await run('이주연 d-3 S1 사실추궁', 'b', 'd-3', 'S1', 'fact_pursuit',
    '이주연 씨, 누수 원인이 본인 집 배수 문제가 아니라고 생각하시는 이유가 무엇입니까?')
  await delay(2000)

  // ═══════════════════════════════════════════
  // 18~19턴: 정한결 S4 (감정), 증거 e-4/e-5
  // ═══════════════════════════════════════════
  console.log('--- 18~19턴: 정한결 S4 + 증거 ---')
  await run('정한결 d-1 S4 공감접근(emotional)', 'a', 'd-1', 'S4', 'empathy_approach',
    '정한결 씨, 당시 심정을 충분히 듣겠습니다. 솔직하게 말씀해 주십시오.')
  await delay(2000)
  await run('정한결 e-4 증거제시', 'a', 'd-3', 'S2', 'evidence_present',
    '정한결 씨, 이 옥상 우수관 막힘 사진과 보수 영수증에 대해 입장을 밝혀 주십시오.',
    `증거: ${e4.name}\n설명: ${e4.description?.slice(0, 120) ?? '옥상 배관 사진과 보수 영수증'}`)
  await delay(2000)

  // ═══════════════════════════════════════════
  // 20턴: 정한결 S5 (고백)
  // ═══════════════════════════════════════════
  console.log('--- 20턴: 정한결 d-1 S5 고백 ---')
  await run('정한결 d-1 S5 사실추궁(confess)', 'a', 'd-1', 'S5', 'fact_pursuit',
    '정한결 씨, 이제 사실대로 말씀해 주십시오.')
  ...
  for (const r of results.filter(r => !r.error)) {
    ...
    const hasNeighborTerms = /옆집|상대방|504호|403호/.test(resp)
    const honorific = !/[해야겠어궁금해알아몰라싶어거야건데거든]$/.test(resp.replace(/\s+/g, ''))
    console.log(`  #${r.num} ${r.label}: wrong=${hasWrongTerms ? 'LEAK!' : 'ok'} neighbor=${hasNeighborTerms ? 'ok' : '-'} honorific=${honorific ? 'ok' : 'WARN'}`)
  }
  ...
  // 결과 저장
  const outPath = path.join(__dirname, '..', 'docs/ref/리뉴얼참고/thread-packages/thread-E/reports/neighbor-01-playthrough-results.txt')
