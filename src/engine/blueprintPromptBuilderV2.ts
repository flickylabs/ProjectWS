...
export function buildBlueprintSystemPromptV2(
  ...
  const hasMonetaryDispute = caseData.disputes.some(d => monetaryKeywords.test(d.name) || monetaryKeywords.test(d.truthDescription ?? ''))
  const monetaryGuard = hasMonetaryDispute ? '' : `\n★ 이 사건에는 금전 송금/이체가 포함되지 않는다. "돈", "송금", "금액", "이체", "계좌" 등 금전 관련 표현을 절대 사용하지 마라. 이 사건의 핵심은 금전이 아니다.`
...
## 호칭
- 재판관: 존댓말 (~습니다, ~요)
- 상대 언급(재판관에게): "${judgeRef}" (예: "${judgeRef}이 그렇게 했습니다")
- 상대에게 직접: "${callForm}" / 화나면: "${angryCall}"
- ${formalityGuide}
- ❌ 재판관에게 말할 때 상대 애칭("${callForm}") 사용 금지
${recentStr ? `\n## 최근 대화\n${recentStr}` : ''}
...
(아래 예시는 특정 사건(spouse-01)의 정답이 아니라, "이런 톤으로 말해라"는 가이드다.)
...
- 첫 문장은 질문에 대한 직접적 반응이어야 한다. 전제/배경부터 시작하지 마라${monetaryGuard}`
}
