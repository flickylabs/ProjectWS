 ...
 * ResonanceLayer
 * ──────────────
 * 공명선(번개) 오버레이. pendingResonances 큐를 구독해 각 요청마다
 * SVG jagged zigzag path를 그린 뒤 0.9초 후 자동 제거.
 */
...
function AuraBolt({ request }: { request: AuraRequest }) {
  ...
  }, [request.id, request.targetSelector])

  if (!paths) return null
  return (
    <g className="pc-aura-group">
      {paths.map((d, i) => (
...
function ResonanceBolt({ request }: { request: ResonanceRequest }) {
  ...
  useEffect(() => {
    ...
    setPath(buildJaggedPath(x1, y1, x2, y2))
    ...
    return () => window.clearTimeout(timer)
  }, [request.id, request.fromSelector, request.toSelector])
