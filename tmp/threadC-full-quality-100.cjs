...
const MANIFEST_PATH = path.join(ROOT, 'scripts', 'manifests', 'thread-a-current-new100.json')
const CASE_IDS = JSON.parse(fs.readFileSync(MANIFEST_PATH, 'utf8')).cases
...
function analyzeLevel4(caseId, caseData, bundle) {
  const categories = {
    '4-A': [],
    '4-B': [],
    '4-C': [],
  ...
  if (evidenceSample && normalizeSkeleton(evidenceSample).includes('K K K K')) {
  ...
  if (/사건 흐름을 시점별로 나누어 설명합니다/u.test(witnessFull)) {
    categories['4-D'].push(buildIssue('4-D2', '증인 증언이 사건 고유 사실보다 공통 메타 서술에 가까움', witnessFull))
  }

  if (aftermathSample && /본인 측 책임을 더 무겁게 적습니다/u.test(aftermathSample)) {
...
function analyzeCase(caseId, globalMeta) {
  ...
  const level4 = analyzeLevel4(caseId, caseData, bundle)
  if (globalMeta.genericToJudgeCases.has(caseId) && level4.statuses['4-A'] === 'PASS') {
    level4.statuses['4-A'] = 'WARN'
  }
  if (level4.statuses['4-G'] === 'PASS' && globalMeta.crossCaseScaffoldIssues.s0.has(caseId)) {
    level4.statuses['4-G'] = 'WARN'
    level4.issues['4-G'].push(buildIssue('4-G1', '케이스 간 raw 텍스트는 달라졌지만 normalized scaffold가 공통 템플릿과 동일함', level4.samples.interrogation.a_s0))
  }
  if (level4.statuses['4-H'] === 'PASS' && globalMeta.crossCaseScaffoldIssues.aftermath.has(caseId)) {
    level4.statuses['4-H'] = 'WARN'
    level4.issues['4-H'].push(buildIssue('4-H1', 'aftermath가 사건마다 다른 명사만 넣은 공통 골격에 가까움', level4.samples.aftermath))
  }
  if (level4.statuses['4-D'] === 'PASS' && globalMeta.crossCaseScaffoldIssues.witness.has(caseId)) {
    level4.statuses['4-D'] = 'WARN'
    level4.issues['4-D'].push(buildIssue('4-D1', '문장 수는 맞지만 witness full 진술 골격이 다른 사건과 거의 동일함', level4.samples.witnessFull))
  }
...
function computeGlobalMeta() {
  const genericToJudgeCases = new Set()
  const s0SkeletonMap = new Map()
  const aftermathSkeletonMap = new Map()
  const witnessSkeletonMap = new Map()
  ...
  for (const caseId of CASE_IDS) {
    ...
    const aJudge = caseData.duo.partyA.callTerms?.toJudge || ''
    const bJudge = caseData.duo.partyB.callTerms?.toJudge || ''
    if (/^해당/.test(aJudge) || /^해당/.test(bJudge)) genericToJudgeCases.add(caseId)

    const s0 = pickText(getEntryByKey(bundle, 'interrogation', 'a|d-1|S0|fact_pursuit'))
    const aftermath = pickText(bundle.channels.aftermath.entries[0])
    const witnessFull = pickText(getEntry(bundle, 'witness', (entry) => entry.key.endsWith('|full')))

    const addToMap = (map, text, id) => {
      const key = normalizeSkeleton(text)
      map.set(key, [...(map.get(key) || []), id])
    }
    addToMap(s0SkeletonMap, s0, caseId)
    addToMap(aftermathSkeletonMap, aftermath, caseId)
    addToMap(witnessSkeletonMap, witnessFull, caseId)
  }

  const scaffoldCases = (map) => new Set(
    [...map.values()]
  ...
  return {
    genericToJudgeCases,
    crossCaseScaffoldIssues: {
      s0: scaffoldCases(s0SkeletonMap),
      aftermath: scaffoldCases(aftermathSkeletonMap),
      witness: scaffoldCases(witnessSkeletonMap),
    },
    counts: {
      genericToJudge: genericToJudgeCases.size,
      normalizedS0Groups: s0SkeletonMap.size,
      normalizedAftermathGroups: aftermathSkeletonMap.size,
      normalizedWitnessGroups: witnessSkeletonMap.size,
    },
  }
}

function buildSummary(results, globalMeta) {
  const summary = {
    ...
    regressionChecks: {
      callTermsNoCounterparty: results.filter((result) => !['상대방'].includes(result.level4.metrics?.callTerms?.a || '') && !['상대방'].includes(result.level4.metrics?.callTerms?.b || '')).length,
      genericToJudge: globalMeta.counts.genericToJudge,
      rawS0Unique: new Set(results.map((result) => result.level4.samples.interrogation.a_s0)).size,
      normalizedS0Groups: globalMeta.counts.normalizedS0Groups,
      ...
      normalizedAftermathGroups: globalMeta.counts.normalizedAftermathGroups,
    },
...
function buildMarkdown(results, summary) {
  const lines = []
  lines.push('# Thread-C Full Quality Test — 100 Cases')
  lines.push('')
  ...
  for (const result of results) {
    ...
    for (const issue of result.level4.issues['4-A'].slice(0, 2)) {
      lines.push(`  - ${issue.code.replace(/^FAIL:/, '')}: ${issue.detail}${issue.quote ? ` | ${issue.quote}` : ''}`)
    }
    lines.push(`- 4-B: ${result.level4.statuses['4-B']}`)
    lines.push(`- 4-C: ${result.level4.statuses['4-C']}`)
    lines.push(`- 4-D: ${result.level4.statuses['4-D']}`)
    for (const issue of result.level4.issues['4-D'].slice(0, 2)) {
      lines.push(`  - ${issue.code.replace(/^FAIL:/, '')}: ${issue.detail}${issue.quote ? ` | ${issue.quote}` : ''}`)
    }
    lines.push(`- 4-E: ${result.level4.statuses['4-E']}`)
    for (const issue of result.level4.issues['4-E'].slice(0, 2)) {
      lines.push(`  - ${issue.code.replace(/^FAIL:/, '')}: ${issue.detail}${issue.quote ? ` | ${issue.quote}` : ''}`)
    }
    lines.push(`- 4-F: ${result.level4.statuses['4-F']}`)
    for (const issue of result.level4.issues['4-F'].slice(0, 2)) {
      lines.push(`  - ${issue.code.replace(/^FAIL:/, '')}: ${issue.detail}${issue.quote ? ` | ${issue.quote}` : ''}`)
    }
    lines.push(`- 4-G: ${result.level4.statuses['4-G']}`)
    for (const issue of result.level4.issues['4-G'].slice(0, 2)) {
      lines.push(`  - ${issue.code.replace(/^FAIL:/, '')}: ${issue.detail}${issue.quote ? ` | ${issue.quote}` : ''}`)
    }
    lines.push(`- 4-H: ${result.level4.statuses['4-H']}`)
    for (const issue of result.level4.issues['4-H'].slice(0, 2)) {
      lines.push(`  - ${issue.code.replace(/^FAIL:/, '')}: ${issue.detail}${issue.quote ? ` | ${issue.quote}` : ''}`)
    }
    lines.push(`- Samples:`)
...
function buildBatchMarkdown(results, batchIndex) {
  const header = [`# Thread-C Full Quality Test — Batch ${batchIndex}`, '']
  for (const result of results) {
    header.push(`## ${result.caseId}`)
    header.push(`- Level 1: ${result.levelStatuses.level1}`)
    header.push(`- Level 2: ${result.levelStatuses.level2}`)
    header.push(`- Level 3: ${result.levelStatuses.level3}`)
    header.push(`- Level 4: ${result.levelStatuses.level4}`)
    header.push(`- Level 5: ${result.levelStatuses.level5}`)
    header.push(`- 4-A: ${result.level4.statuses['4-A']} | ${result.level4.issues['4-A'][0]?.detail || ''}`)
    header.push(`- 4-D: ${result.level4.statuses['4-D']} | ${result.level4.issues['4-D'][0]?.detail || ''}`)
    header.push(`- 4-E: ${result.level4.statuses['4-E']} | ${result.level4.issues['4-E'][0]?.detail || ''}`)
    header.push(`- 4-F: ${result.level4.statuses['4-F']} | ${result.level4.issues['4-F'][0]?.detail || ''}`)
    header.push(`- 4-G: ${result.level4.statuses['4-G']} | ${result.level4.issues['4-G'][0]?.detail || ''}`)
    header.push(`- 4-H: ${result.level4.statuses['4-H']} | ${result.level4.issues['4-H'][0]?.detail || ''}`)
    header.push('')
  }
  return header.join('\n')
}
...
function main() {
  ...
  const summary = buildSummary(results, globalMeta)

  const jsonPath = path.join(ROOT, 'tmp', 'threadC-full-quality-100-report.json')
  const mdPath = path.join(ROOT, 'tmp', 'threadC-full-quality-100-report.md')
  ...
  for (let index = 0; index < 5; index += 1) {
    ...
    const batchPath = path.join(ROOT, 'tmp', `threadC-full-quality-100-batch-${index + 1}.md`)
